...
class ThresholdStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # 8. Sagemaker Role & Model Deployment
        sagemaker_role = iam.Role(self, "ThresholdSageMakerRole",
            assumed_by=iam.ServicePrincipal("sagemaker.amazonaws.com")
        )
        data_lake.grant_read(sagemaker_role)

        model_deployment = s3deploy.BucketDeployment(self, "DeployModel",
            sources=[s3deploy.Source.asset("../ml/model_asset")],
            destination_bucket=data_lake,
            destination_key_prefix="ml/"
        )
        ...
        sagemaker_model.node.add_dependency(model_deployment)
        sagemaker_model.node.add_dependency(data_lake)
